...
SCALAR_METRIC_ORDER: List[str] = ["erds_band", "median_freq", "auc"]
...
def _log(msg: str) -> None:
    """
    Print an analysis progress message to standard output.
...
def _run_sort_key(path: Path) -> int:
    """
    Return a natural sort key so that run10 follows run9 (not run1).

    Extracts the integer that follows ``run`` in the file stem.

    Args:
        path (Path): path to a per-run pickle whose stem contains 'run<N>'.

    Returns:
        int: the run number, or 0 if no 'run<N>' token is present.
    """
...
def discover_pickles(processed_root: Path, condition: str) -> Dict[str, List[Path]]:
    ...
    Args:
        processed_root (Path): root directory of the processed pickles (contains
            the ``motor_execution`` / ``motor_imagery`` subfolders).
        condition (str): 'ME' or 'MI'; selects the condition subfolder.

    Returns:
        Dict[str, List[Path]]: mapping from subject id (the 'Sxx' folder name,
        e.g. 'S02') to its run pickles, ordered run1 ... run10. Empty if the
...
def _concat_metrics(metrics_list: List[Dict]) -> Dict:
    """
    Concatenate a subject's per-run metrics dicts along the epoch axis.

    Merges the nested ``metrics`` dicts (one per run) so that every band's
    baseline/active scalars and the band ERD/ERS are stacked over epochs (axis 0),
    keeping the shared ``freqs_band`` axis.

    Args:
        metrics_list (List[Dict]): the ``metrics`` dicts of the runs to merge, as
            produced by ``eeg_pipeline.extract_band_metrics``.

    Returns:
        Dict: a single metrics dict with the same nested structure, concatenated
        over the epoch axis.
    """
    out: Dict = {}
...
def load_subject(paths: List[Path]) -> Dict:
    ...
    The per-run curves (``psd_baseline``, ``psd_active``, ``erds_curve``),
    events, and all nested band scalars are stacked so the subject is represented
    as a single combined recording of ~60 trials per class. The ``freqs`` and
    ``ch_names`` axes are asserted identical across runs.

    Args:
        paths (List[Path]): the run pickles of one subject (one condition).

    Returns:
        Dict: a combined recording dict with keys ``psd_baseline``,
        ``psd_active``, ``erds_curve``, ``events`` (all concatenated over epochs),
        ``metrics`` (merged), ``freqs``, ``ch_names``, and ``condition``.
    """
...
def load_all_subjects(processed_root: Path) -> Dict[Tuple[str, str], Dict]:
    ...
    Args:
        processed_root (Path): root directory of the processed pickles.

    Returns:
        Dict[Tuple[str, str], Dict]: mapping from ``(condition, subject_id)`` to
        that subject's combined recording dict (see ``load_subject``).
    """
...
def _mean_ci(data: np.ndarray, axis: int = 0, conf: float = 0.95
    ...
    """
    Compute the t-based mean and confidence bounds along an axis.

    The confidence interval is ``mean +/- t(0.5+conf/2, n-1) * SEM``. For n < 2
    the bounds collapse to the mean. NaNs are ignored.

    Args:
        data (np.ndarray): input array; statistics are taken along ``axis``.
        axis (int): axis along which to reduce. Default 0.
        conf (float): confidence level in (0, 1). Default 0.95.

    Returns:
        Tuple[np.ndarray, np.ndarray, np.ndarray]: (mean, ci_lo, ci_hi), each
        with ``axis`` removed from the shape of ``data``.
    """
    n = data.shape[axis]
...
def _class_names(events: np.ndarray) -> np.ndarray:
    """
    Map the event-code column of an events array to class-name strings.

    Args:
        events (np.ndarray): events array of shape (n_epochs, 3); the third
            column holds the integer event codes.

    Returns:
        np.ndarray: array of shape (n_epochs,) with the class-name string of each
        epoch (via ``CODE_TO_EVENT``).
    """
...
def build_scalar_dataframe(subjects: Dict[Tuple[str, str], Dict]) -> pd.DataFrame:
    ...
    Produces one row per (subject, condition, class, channel, band, epoch_idx).
    All seven classes and all five recorded channels are kept; filtering to the
    analysis subset happens at plot time. The columns are: ``subject``,
    ``condition``, ``class``, ``channel``, ``band``, ``epoch_idx``,
    ``auc_baseline``, ``auc_active``, ``medfreq_baseline``, ``medfreq_active``,
    and ``erds_band``.

    Args:
        subjects (Dict[Tuple[str, str], Dict]): mapping from
            ``(condition, subject_id)`` to a combined recording dict (see
            ``load_subject``).

    Returns:
        pd.DataFrame: the long-form scalar table, with the identifier columns
        cast to the ``category`` dtype.
    """
...
def build_curve_arrays(subjects: Dict[Tuple[str, str], Dict]) -> Dict:
    ...
    Returns:
        Dict: a curves backbone with keys
            - 'freqs' (np.ndarray): frequency axis, (n_freqs,);
            - 'channels' (List[str]): all recorded channels, in order;
            - 'classes' (List[str]): CURVE_CLASSES (5 analysis classes + 'movement');
            - 'subjects' (Dict[str, List[str]]): subject ids per condition;
            - 'data' (Dict[Tuple[str, str], Dict[str, np.ndarray]]): per
              ``(condition, curve_type)`` a dict with 'mean', 'ci_lo', 'ci_hi'.
    """
...
def save_backbone(df: pd.DataFrame, curves: Dict, backbone_dir: Path) -> None:
    """
    Persist the analysis backbone to disk.

    Writes the scalar DataFrame as ``scalars.pkl`` and the curve arrays as a
    compressed ``curves.npz`` (the nested curves dict is flattened into
    ``data__<condition>__<curve_type>__<stat>`` keys plus axis metadata).

    Args:
        df (pd.DataFrame): the scalar DataFrame from ``build_scalar_dataframe``.
        curves (Dict): the curves backbone from ``build_curve_arrays``.
        backbone_dir (Path): directory to write into; created if missing.
...
def load_backbone(backbone_dir: Path) -> Tuple[pd.DataFrame, Dict]:
    ...
    Reads ``scalars.pkl`` and ``curves.npz`` and rebuilds the ``curves`` dict
    (inverting the flattening applied by ``save_backbone``).

    Args:
        backbone_dir (Path): directory containing ``scalars.pkl`` and
            ``curves.npz``.

    Returns:
        Tuple[pd.DataFrame, Dict]: the (scalar DataFrame, curves backbone) pair,
        matching the outputs of ``build_scalar_dataframe`` and
        ``build_curve_arrays``.
    """
...
def _channel_indices(channels: List[str], wanted: List[str]) -> List[int]:
    """
    Return the positions of ``wanted`` channels within ``channels``.

    Args:
        channels (List[str]): the full ordered list of channel names.
        wanted (List[str]): the channel names to locate.

    Returns:
        List[int]: index of each wanted channel in ``channels`` (same order as
        ``wanted``).
    """
...
def get_curve(curves: Dict, condition: str, curve_type: str, klass: str,
    ...
    At ``level='subject'`` the subject's own mean and within-subject CI (across
    that subject's epochs) are returned and ``subject`` is required; at
    ``level='group'`` the mean and CI are computed across the subject-means.

    Args:
        curves (Dict): the curves backbone (from ``build_curve_arrays`` /
            ``load_backbone``).
        condition (str): 'ME' or 'MI'.
        curve_type (str): one of CURVE_TYPES ('psd_baseline', 'psd_active',
            'erds_curve').
        klass (str): a class in CURVE_CLASSES (analysis classes or 'movement').
        level (str): 'subject' or 'group'.
        subject (Optional[str]): subject id; required when ``level='subject'``.

    Returns:
        Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, int]:
        (freqs, mean, lo, hi, n), where mean/lo/hi have shape
        (n_channels, n_freqs) over all recorded channels and ``n`` is the number
        of contributing subjects (group level) or 1 (subject level).
    """
...
def _add_band_shading(ax) -> None:
    """
    Shade the alpha and beta bands as light vertical spans on an axis.

    Args:
        ax (matplotlib.axes.Axes): the axis to shade in place.
...
def _style_curve_ax(ax, title: str, ylabel: str, shade: bool, zero_line: bool) -> None:
    """
    Apply the shared styling to a single curve axis.

    Optionally shades the alpha/beta bands and draws a 0 dB reference line, then
    sets the title, axis labels, and tick sizes.

    Args:
        ax (matplotlib.axes.Axes): the axis to style in place.
        title (str): panel title.
        ylabel (str): y-axis label.
        shade (bool): if True, shade the alpha/beta bands.
        zero_line (bool): if True, draw a dashed horizontal line at y = 0.
...
def _class_title(klass: str) -> str:
    """
    Return a human-readable Title-case label for a class key.

    The pooled label maps to 'Movement'; other keys have underscores replaced by
    spaces and are title-cased (e.g. 'elbow_flexion' -> 'Elbow Flexion').

    Args:
        klass (str): a class key (an analysis class or POOLED_LABEL).

    Returns:
        str: the presentation label.
    """
    if klass == POOLED_LABEL:
        return "Movement"
...
def _plot_psd_grid(curves: Dict, condition: str, rows: List[str], level: str,
    ...
    """
    Draw the shared PSD overlay grid (rows x channels, baseline vs active).

    Each panel overlays the baseline and active PSD curves (with 95% CI bands)
    for one class/channel; a single legend is placed at ``leg_anchor``.

    Args:
        curves (Dict): the curves backbone.
        condition (str): 'ME' or 'MI'.
        rows (List[str]): class keys to place on the grid rows.
        level (str): 'subject' or 'group'.
        subject (Optional[str]): subject id when ``level='subject'``.
        leg_anchor (Tuple[float, float]): figure-fraction anchor for the legend.
...
def plot_psd_pooled(curves: Dict, condition: str, level: str,
    ...
    """
    PSD figure (pooled): rows {Movement, Rest} x channels, baseline vs active.
...
def plot_psd_per_class(curves: Dict, condition: str, level: str,
                       subject: Optional[str] = None):
    """
    PSD figure (per-class): rows = 5 analysis classes x channels.
...
def _plot_erds_row(curves: Dict, condition: str, classes: List[str], level: str,
                   subject: Optional[str], leg_anchor: Tuple[float, float], title: str):
    """
    Draw the shared ERD/ERS single-row grid (1 x channels, classes overlaid).

    Each panel overlays the ERD/ERS(f) curves (with 95% CI bands) of the given
    classes for one channel, with alpha/beta shading and a 0 dB reference line; a
    single legend is placed at ``leg_anchor``.

    Args:
        curves (Dict): the curves backbone.
        condition (str): 'ME' or 'MI'.
        classes (List[str]): class keys to overlay in each panel.
        level (str): 'subject' or 'group'.
        subject (Optional[str]): subject id when ``level='subject'``.
        leg_anchor (Tuple[float, float]): figure-fraction anchor for the legend.
        title (str): figure super-title.
...
def plot_erds_pooled(curves: Dict, condition: str, level: str,
                     subject: Optional[str] = None):
    """
    ERD/ERS figure (pooled): Movement vs Rest overlaid, 1 x channels.
...
def plot_erds_per_class(curves: Dict, condition: str, level: str,
                        subject: Optional[str] = None):
    """
    ERD/ERS figure (per-class): all 5 analysis classes overlaid, 1 x channels.
...
def _scalar_plot_frame(df: pd.DataFrame, metric: str, level: str,
    ...
    """
    Slice or aggregate the master DataFrame for one scalar box plot.

    Filters to the analysis class/channel subset. At ``level='group'`` the rows
    are collapsed to one value per subject (the subject mean) so each box spans
    the subject-means; at ``level='subject'`` the raw epoch rows of one subject
    are returned so each box spans that subject's ~60 epochs.

    Args:
        df (pd.DataFrame): the master scalar DataFrame.
        metric (str): short metric name (key of METRIC_COLUMNS).
        level (str): 'group' or 'subject'.
        subject (Optional[str]): subject id; required when ``level='subject'``.

    Returns:
        pd.DataFrame: the filtered (subject level) or subject-mean aggregated
        (group level) rows for plotting.
    """
...
def plot_scalar_box(df: pd.DataFrame, metric: str, level: str,
                    subject: Optional[str] = None):
    """
    Draw the box plot for one scalar metric.

    The grid has band rows and channel columns, with class on the x-axis and
    condition as hue. Only the x-axis is shared across facets (each panel
    autoscales its y). Class names are shown as short abbreviations
    (EF/EE/HC/HO/Rest); band rows and the y-axis carry Pascal-case labels; and the
    condition legend sits in the figure's upper-right corner.

    Args:
        df (pd.DataFrame): the master scalar DataFrame.
        metric (str): short metric name (key of METRIC_COLUMNS).
        level (str): 'group' (box per subject-mean) or 'subject' (box per epoch).
        subject (Optional[str]): subject id; required when ``level='subject'``.

    Returns:
        seaborn.axisgrid.FacetGrid: the grid (use ``.figure`` to save).
    """
    col = METRIC_COLUMNS[metric]
...
def summary_tables(df: pd.DataFrame) -> pd.DataFrame:
    """
    Build the descriptive summary table across subject-means.

    Epochs are first collapsed to one value per subject, and then the mean, SD,
    and t-based 95% CI are computed across subjects for every scalar metric,
    grouped by (condition, class, channel, band). Subjects are the unit of
    observation, and the analysis class/channel subset is used.

    Args:
        df (pd.DataFrame): the master scalar DataFrame.

    Returns:
        pd.DataFrame: long-form table with columns ``condition``, ``class``,
        ``channel``, ``band``, ``metric``, ``mean``, ``sd``, ``n``, ``ci_lo``,
        ``ci_hi``.
    """
...
def _save_fig(fig, path: Path, transparent: bool = True) -> None:
    """
    Save a figure to disk (creating parent directories) and close it.

    The format is inferred from the file extension, ``bbox_inches='tight'`` keeps
    outer legends, and the figure is closed to free memory.

    Args:
        fig (matplotlib.figure.Figure): the figure to save.
        path (Path): output path; its parent directory is created if missing.
        transparent (bool): if True, save with a transparent background.
            Default True.
...
def main_analysis(
    ...
    """
    Run the full descriptive analysis end to end.

    Discovers the processed pickles, builds (or reloads) the backbone, and then
    generates the curve figures, scalar box plots, and the descriptive summary
    table under ``results_root``.

    Args:
        processed_root (Optional[str]): root of the processed pickles; defaults
            to ``<project_root>/data/data_processed``.
        results_root (Optional[str]): root for the outputs (backbone, figures,
            tables); defaults to ``<project_root>/results``.
        rebuild (bool): if False (default) and a persisted backbone exists,
            reload it instead of re-reading every per-run pickle.
        per_subject_figs (bool): if True (default), also emit per-subject
            curve/scalar figures; group-level figures are always produced.
